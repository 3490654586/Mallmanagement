        ...
        <el-table-column  label="操作"  width="190">
          <template slot-scope="scope">
            ...
            <el-tooltip  effect="dark" content="删除酒水" placement="top-start">
              <el-button  type="danger" icon="el-icon-delete" size="mini" @click="deleteCommdity(scope.row.id)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
<!--    修改用户对话框-->
    <el-dialog title="修改商品" :visible.sync="putdialogVisible" width="50%">
      <el-form :model="idForm" label-width="100px" class="demo-ruleForm">
        <el-form-item label="酒水厂家">
          <el-input v-model="idForm.commdity_name"></el-input>
        </el-form-item>
        <el-form-item label="酒水数量">
          <el-input v-model="idForm.commdity_number"></el-input>
        </el-form-item>
        <el-form-item label="酒水价格" >
          <el-input v-model="idForm.commdity_moye"></el-input>
        </el-form-item>
     </el-form>
      <span slot="footer" class="dialog-footer">
    <el-button @click="putdialogVisible = false">取 消</el-button>
    <el-button type="primary" @click="putcommdiyty">确 定</el-button>
  </span>
    </el-dialog>
  </div>
</template>
    ...
    export default {
        ...
        methods:{
           ...
           async addcommdiyty(){
            ...
            //编辑酒水
           async showputdialogVisible(ids){
                this.putdialogVisible = true
             const {data:res} = await this.$http.get("http://localhost:8077/comm/getidcommdity",{params:{id:ids}})
                if (res.stres!=200) {
           ...
           async putcommdiyty(){
                 console.log("this.idForm",this.idForm)
                  const {data:res} = await this.$http.put('http://localhost:8077/comm/putcommdity/'+this.idForm.id,{
                      commdity_name:this.idForm.commdity_name,
                      commdity_number:Number(this.idForm.commdity_number),
                      commdity_moye:parseFloat(this.idForm.commdity_moye)
                  }
  )
                   if (res.stare != 200){
                       this.$message.error(res.msg)
           ...
           async deleteCommdity(id){
             const msgconfigm = await this.$confirm('此操作将永久删除该商品, 是否继续?', '提示', {
              ...
              if (msgconfigm != "confirm"){
                  return  this.$message.info("您已取消删除此商品")
              }

              const {data:res} = await this.$http.delete('http://localhost:8077/comm/delcommdity/'+ id)
               if (res.stars != 200){
